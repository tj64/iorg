;;; ox-ihtml.el --- iOrg Back-End for Org Export Engine
...
;; This library implements a derived html back-end for Org exporter.
;;
;; It introduces two interactive functions, `org-ihtml-export-as-org'
;; and `org-ihtml-export-to-org', which export, respectively, to
;; a temporary buffer and to a file.
;;
;; A publishing function is also provided: `org-ihtml-publish-to-org'.
...
(org-export-define-backend 'iorg
  '((babel-call . identity)
    ...
    (paragraph . org-ihtml-paragraph)
    (plain-list . org-ihtml-plain-list)
    (plain-text . org-ihtml-plain-text)
    (planning . identity)
    (property-drawer . identity)
    (quote-block . identity)
    (quote-section . identity)
    (radio-target . org-ihtml-radio-target)
    (section . org-ihtml-section)
    ...
    (table . org-ihtml-table)
    (table-cell . org-ihtml-table-cell)
    (table-row . org-ihtml-table-row)
    (target . identity)
    (template . org-ihtml-template)
    (timestamp . org-ihtml-timestamp)
    (underline . identity)
    (verbatim . identity)
  ...
  :menu-entry
  '(?O "Export to Org"
       ((?O "As Org buffer" org-ihtml-export-as-iorg)
	(?o "As Org file" org-ihtml-export-to-iorg)
	(?v "As Org file and open"
	    (lambda (a s v b)
	      (if a (org-ihtml-export-to-iorg t s v b)
		(org-open-file (org-ihtml-export-to-iorg nil s v b))))))))
...
(define-obsolete-variable-alias
  'org-export-htmlized-org-css-url 'org-ihtml-htmlized-css-url "24.4")
(defcustom org-ihtml-htmlized-css-url nil
...
`org-ihtml-export-as-org' will include a link to this URL if the
...
(defun org-ihtml-template (contents info)
...
(defun org-ihtml-headline (headline contents info)
...
(defun org-ihtml-section (headline contents info)
  "Transcode SECTION element into iOrg syntax.
CONTENTS is its contents, as a string or nil.  INFO is ignored.")

(defun org-ihtml-paragraph (headline contents info)
  "Transcode PARAGRAPH element into iOrg syntax.
CONTENTS is its contents, as a string or nil.  INFO is ignored.")

(defun org-ihtml-plain-list (headline contents info)
  "Transcode PLAIN-LIST element into iOrg syntax.
CONTENTS is its contents, as a string or nil.  INFO is ignored.")

(defun org-ihtml-radio-target (headline contents info)
  "Transcode RADIO-TARGET element into iOrg syntax.
CONTENTS is its contents, as a string or nil.  INFO is ignored.")

(defun org-ihtml-table (headline contents info)
  "Transcode TABLE element into iOrg syntax.
CONTENTS is its contents, as a string or nil.  INFO is ignored.")

(defun org-ihtml-table-cell (headline contents info)
  "Transcode TABLE-CELL element into iOrg syntax.
CONTENTS is its contents, as a string or nil.  INFO is ignored.")

(defun org-ihtml-table-row (headline contents info)
  "Transcode TABLE-ROW element into iOrg syntax.
CONTENTS is its contents, as a string or nil.  INFO is ignored.")

(defun org-ihtml-timestamp (headline contents info)
  "Transcode TIMESTAMP element into iOrg syntax.
CONTENTS is its contents, as a string or nil.  INFO is ignored.")

(defun org-ihtml-plain-text (headline contents info)
  "Transcode PLAIN-TEXT element into iOrg syntax.
CONTENTS is its contents, as a string or nil.  INFO is ignored.")

;;;; Filter Functions

(defun org-ihtml-final-function (contents backend info)
...
;;;###autoload
(defun org-ihtml-export-as-iorg (&optional async subtreep visible-only ext-plist)
...
;;;###autoload
(defun org-ihtml-export-to-iorg (&optional async subtreep visible-only ext-plist)
...
;;;###autoload
(defun org-ihtml-publish-to-iorg (plist filename pub-dir)
...
Return output file name."
  ...
  (when (plist-get plist :htmlized-source)
	...
	(when org-ihtml-htmlized-css-url
	  (goto-char (point-min))
	  (and (re-search-forward
		...
		(format
		 ...
		 org-ihtml-htmlized-css-url) t t)))
